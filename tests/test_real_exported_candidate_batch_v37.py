...
REPORT = ROOT / "artifacts" / "real_exported_candidate_batch_v37_report.json"
RECEIPT = ROOT / "artifacts" / "real_exported_candidate_batch_v37_receipt.json"
TRACES = ROOT / "artifacts" / "real_exported_candidate_batch_v37_traces.jsonl"


class TestRealExportedCandidateBatchV37(unittest.TestCase):
    def test_real_exported_candidate_batch_gates(self):
        subprocess.run(
            [sys.executable, "scripts/v3_7/evaluate_real_exported_candidate_batch_v37.py"],
            cwd=ROOT,
            check=True,
        )
