...
DATA_PATH = ROOT / "data" / "v9_3" / "runtime_replay_cli_cases.jsonl"
REPORT_PATH = ROOT / "artifacts" / "runtime_replay_cli_report.json"
RECEIPT_PATH = ROOT / "artifacts" / "runtime_replay_cli_receipt.json"
...
def run_case(case: dict[str, object]) -> dict[str, object]:
    ...
    if expected_exit_code == 2:
        action_ok = payload.get("action") == "invalid_input"
    else:
        action_ok = actual_actions == expected_actions and payload.get("action") == "replay_completed"
...
def main() -> int:
    ...
    report = {
        ...
        "runtime_replay_cli_accuracy": passed / len(results) if results else 0.0,
    ...
    receipt = {
        "release": "v9.3.0",
        "receipt_type": "runtime_replay_cli",
        "all_gates_passed": report["all_gates_passed"],
        "case_count": report["case_count"],
        "runtime_replay_cli_accuracy": report["runtime_replay_cli_accuracy"],
        "candidate_graph_contamination_count": report["candidate_graph_contamination_count"],
        "cli_replays_multi_event_sessions": True,
        "invalid_replay_sessions_rejected_safely": True,
