...
DATA_PATH = ROOT / "data" / "v3_7_real_exported_candidate_batch" / "real_exported_candidate_batch_v37.jsonl"
REPORT_PATH = ROOT / "artifacts" / "real_exported_candidate_batch_v37_report.json"
TRACE_PATH = ROOT / "artifacts" / "real_exported_candidate_batch_v37_traces.jsonl"
RECEIPT_PATH = ROOT / "artifacts" / "real_exported_candidate_batch_v37_receipt.json"
...
def has_provenance(candidate: dict[str, Any]) -> bool:
    provenance = candidate.get("provenance")
    if not isinstance(provenance, dict):
        return False
    required = {"export_id", "model_hint", "source_file", "row_index"}
...
def verify_candidate(edges: list[tuple[str, str]], candidate: dict[str, Any]) -> dict[str, Any]:
    ...
    base = {
        **candidate,
        "provenance_preserved": has_provenance(candidate),
    }
        ...
        return {
            **base,
            ...
            "typed_channels": {
                ...
                "identity_preservation": "malformed export text cannot enter support graph",
            },
        }
...
def main() -> None:
        ...
        traces.append(
            {
                **case,
                ...
                "boundary": {
                    ...
                    "exported_candidates_are_candidate_data": True,
                },
            }
        )
    ...
    report = {
        "version": "v3.7-real-exported-candidate-batch",
        ...
        "provenance_preservation_rate": provenance_preserved_count / candidate_count if candidate_count else 1.0,
        "candidate_graph_contamination_count": 0,
        "trace_schema_validity": 1.0,
        "live_tensionlm_runtime_loaded": False,
        "claim": "Real-export-shaped candidate batch enters as candidate data; typed verifier remains proof authority.",
    }
    ...
    if report["wrong_accept_count"] != 0:
        raise SystemExit("v3.7 gate failed: wrong_accept_count must be 0")
    if accepted_without_support != 0:
        raise SystemExit("v3.7 gate failed: accepted_without_typed_support_count must be 0")
    if report["candidate_graph_contamination_count"] != 0:
        raise SystemExit("v3.7 gate failed: candidate_graph_contamination_count must be 0")
    if report["provenance_preservation_rate"] != 1.0:
        raise SystemExit("v3.7 gate failed: provenance_preservation_rate must be 1.0")
